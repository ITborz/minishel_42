...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/08 18:56:29 by iugolin           #+#    #+#             */
...
int	if_builtin(t_cmd *cmd, t_shell *shell)
{
	...
	saved_stdin = dup(0);
	saved_stdout = dup(1);
	if (cmd && cmd->next == NULL && cmd->av[0] != NULL)
	{
		if (cmd->av && check_builtins(cmd->av) == 1)
			return (0);
		if (check_red(cmd, 1) == 1)
			return (1);
		if (is_builtins_exec(cmd->av, 0, cmd, shell) == 0)
		{
